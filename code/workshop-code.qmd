...
# creating object from `drought_exp`
drought_exp_clean <- drought_exp |>
  # makes all data and column titles lowercase and adding underscores
  clean_names() |>
  ...
  # creating new column with full name for treatment type
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |>
  # rearranging the level order for species
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # rearranging the level order for water treatments
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |>
  # selecting columns `species_name`, `water_treatment`, `sla`, and `total_g` for desired use
...
ggplot(data = drought_exp_clean,
       ...
       mapping = aes(x = sla,
                  y = total_g)) +
  # layer two: geom point
  # change size to 2
  geom_point(size = 2) +
  # add title, change x- and y-axis names
  labs(title = "As SLA increases, total mass increases too.",
...
ggplot(data = drought_exp_clean,
  ...
  scale_color_manual(values = c("Drought stressed" = ds_col,
                                "Well watered" = ww_col)) +
  # adding title, changing axes
  labs(title = "On average, well watered plant have greater total mass than drought stressed plants",
       x = "Water treatment",
       y = "Total mass (g)") +
  # removing legend
...
- resudials appear normally distributed
- residuals appear homoscedastic (constant variance)
- no clear outliers in data set influencing model coefficients estimates
...
AICc(
  ...
  model2,
  model3,
  model4, # simplest model that explains the most variation
  model5,
  model6,
  model7
) |> 
...
- `(intercept)` represents the model prediction for plant mass for Lotus scoparius in the drought stressed treatment
